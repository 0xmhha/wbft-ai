...
> **이 문서는 다음 경우에만 로드한다 — 일반 질문에는 로드하지 않는다:**
> 1. **전체 패키지 파일 목록**이 필요할 때 ("go-wbft 고유 코드 맵" 섹션)
> 2. **상세 흐름 다이어그램**이 필요할 때 (유형 4 합의 흐름, 유형 5 거버넌스 패키지 구조 등)
>
> 일반 질문 유형 분류는 `wbft-review-code.md`의 인라인 표를 사용한다.
